...
public class Shop extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (Connection conn = DriverManager.getConnection(
			...
			out.println(
					"<!-- Store Page -->     <section       class='bg-dark text-light p-5 p-lg-0 pt-lg-5 text-center text-sm-start'     >       <div class='container'>         <div class='row g-4'>           <div class='d-sm-flex align-items-center justify-content-between'>             <div>               <h1>Store page</h1>               <p class='lead my-4'>Take a look at what we have available!</p>             </div>           </div>         </div>       </div>     </section>");
			...
			out.println(
					"<section id='store' class='p-5 bg-light text-dark'><div class='container'>");
			while (rset.next()) {
				String name = rset.getString("name");
				String type = rset.getString("type");
				String price = String.valueOf(rset.getInt("price"));
				String qty = String.valueOf(rset.getInt("qty"));
				...
				if (qty.equals("0")) {
					out.print(
							" alt=''/></div><div class='col-md text-center'><input type='hidden' name='name' value='"
									+ name + "''><input type='hidden' name='type' value='" + type
									+ "''><input type='hidden' name='price' value='" + price
									+ "''><input type='hidden' name='qty' value='" + qty
									+ "''><input type='hidden' name='url' value='" + url
									+ "''><input type='submit' value='Out of stock'></div></div>");
				} else {
					out.print(
							" alt=''/></div><div class='col-md text-center'><form method='post' action='purchase'><input type='hidden' name='name' value='"
									+ name + "''><input type='hidden' name='type' value='" + type
									+ "''><input type='hidden' name='price' value='" + price
									+ "''><input type='hidden' name='qty' value='" + qty
									+ "''><input type='hidden' name='url' value='" + url
									+ "''><input type='submit' value='Buy'></form></div></div>");
